...
interface IERC20Errors {
    ...
    /**
     * @dev Indicates a failure with the `spender`тАЩs `allowance`. Used in transfers.
...
interface IERC721Errors {
    ...
    /**
     * @dev Indicates a failure with the `operator`тАЩs approval. Used in transfers.
...
library Math {
     ...
     *
     * The result is stored in two 256 variables such that sum = high * 2┬▓тБ╡тБ╢ + low.
     */
     ...
     *
     * The result is stored in two 256 variables such that product = high * 2┬▓тБ╡тБ╢ + low.
     */
    function mul512(uint256 a, uint256 b) internal pure returns (uint256 high, uint256 low) {
        // 512-bit multiply [high low] = x * y. Compute the product mod 2┬▓тБ╡тБ╢ and mod 2┬▓тБ╡тБ╢ - 1, then use
        // the Chinese Remainder Theorem to reconstruct the 512 bit result. The result is stored in two 256
        // variables such that product = high * 2┬▓тБ╡тБ╢ + low.
        assembly ("memory-safe") {
    ...
    /**
     * @dev Unsigned saturating addition, bounds to `2┬▓тБ╡тБ╢ - 1` instead of overflowing.
     */
    ...
    /**
     * @dev Unsigned saturating multiplication, bounds to `2┬▓тБ╡тБ╢ - 1` instead of overflowing.
     */
    ...
    function mulDiv(uint256 x, uint256 y, uint256 denominator) internal pure returns (uint256 result) {
        unchecked {
            ...
            // Make sure the result is less than 2┬▓тБ╡тБ╢. Also prevents denominator == 0.
            if (denominator <= high) {
            ...
            assembly ("memory-safe") {
                ...
                // Flip twos such that it is 2┬▓тБ╡тБ╢ / twos. If twos is zero, then it becomes one.
            ...
            // Invert denominator mod 2┬▓тБ╡тБ╢. Now that denominator is an odd number, it has an inverse modulo 2┬▓тБ╡тБ╢ such
            // that denominator * inv тЙб 1 mod 2┬▓тБ╡тБ╢. Compute the inverse by starting with a seed that is correct for
            // four bits. That is, denominator * inv тЙб 1 mod 2тБ┤.
            uint256 inverse = (3 * denominator) ^ 2;

            // Use the Newton-Raphson iteration to improve the precision. Thanks to Hensel's lifting lemma, this also
            // works in modular arithmetic, doubling the correct bits in each step.
            inverse *= 2 - denominator * inverse; // inverse mod 2тБ╕
            inverse *= 2 - denominator * inverse; // inverse mod 2┬╣тБ╢
            inverse *= 2 - denominator * inverse; // inverse mod 2┬│┬▓
            inverse *= 2 - denominator * inverse; // inverse mod 2тБ╢тБ┤
            inverse *= 2 - denominator * inverse; // inverse mod 2┬╣┬▓тБ╕
            inverse *= 2 - denominator * inverse; // inverse mod 2┬▓тБ╡тБ╢

            // Because the division is now exact we can divide by multiplying with the modular inverse of denominator.
            // This will give us the correct result modulo 2┬▓тБ╡тБ╢. Since the preconditions guarantee that the outcome is
            // less than 2┬▓тБ╡тБ╢, this is the final result. We don't need to compute the high bits of the result and high
    ...
    function invMod(uint256 a, uint256 n) internal pure returns (uint256) {
        unchecked {
            ...
            // ax + ny = 1
            // ax = 1 + (-y)n
            // ax тЙб 1 (mod n) # x is the inverse of a modulo n
     ...
     * prime, then `a**(p-1) тЙб 1 mod p`. As a consequence, we have `a * a**(p-2) тЙб 1 mod p`, which means that
    ...
    function sqrt(uint256 a) internal pure returns (uint256) {
        unchecked {
            ...
            // of the target. (i.e. `2**(e-1) тЙд sqrt(a) < 2**e`). We know that `e тЙд 128` because `(2┬╣┬▓тБ╕)┬▓ = 2┬▓тБ╡тБ╢` is
            // bigger than any uint256.
            //
            // By noticing that
            // `2**(e-1) тЙд sqrt(a) < 2**e тЖТ (2**(e-1))┬▓ тЙд a < (2**e)┬▓ тЖТ 2**(2*e-2) тЙд a < 2**(2*e)`
            ...
            // We now have x_n such that `x_n = 2**(e-1) тЙд sqrt(a) < 2**e = 2 * x_n`. This implies ╬╡_n тЙд 2**(e-1).
            //
            // We can refine our estimation by noticing that the middle of that interval minimizes the error.
            // If we move x_n to equal 2**(e-1) + 2**(e-2), then we reduce the error to ╬╡_n тЙд 2**(e-2).
            // This is going to be our x_0 (and ╬╡_0)
            xn = (3 * xn) >> 1; // ╬╡_0 := | x_0 - sqrt(a) | тЙд 2**(e-2)
            ...
            //              = ((x_n┬▓ + a) / (2 * x_n))┬▓ - a
            //              = (x_nтБ┤ + 2 * a * x_n┬▓ + a┬▓) / (4 * x_n┬▓) - a
            //              = (x_nтБ┤ + 2 * a * x_n┬▓ + a┬▓ - 4 * a * x_n┬▓) / (4 * x_n┬▓)
            //              = (x_nтБ┤ - 2 * a * x_n┬▓ + a┬▓) / (4 * x_n┬▓)
            //              = (x_n┬▓ - a)┬▓ / (2 * x_n)┬▓
            //              = ((x_n┬▓ - a) / (2 * x_n))┬▓
            //              тЙе 0
            // Which proves that for all n тЙе 1, sqrt(a) тЙд x_n
            //
            ...
            // ╬╡_1 = ╬╡_0┬▓ / | (2 * x_0) |
            //     тЙд (2**(e-2))┬▓ / (2 * (2**(e-1) + 2**(e-2)))
            //     тЙд 2**(2*e-4) / (3 * 2**(e-1))
            //     тЙд 2**(e-3) / 3
            //     тЙд 2**(e-3-log2(3))
            //     тЙд 2**(e-4.5)
            //
            // For the following iterations, we use the fact that, 2**(e-1) тЙд sqrt(a) тЙд x_n:
            // ╬╡_{n+1} = ╬╡_n┬▓ / | (2 * x_n) |
            //         тЙд (2**(e-k))┬▓ / (2 * 2**(e-1))
            //         тЙд 2**(2*e-2*k) / 2**e
            //         тЙд 2**(e-2*k)
            xn = (xn + a / xn) >> 1; // ╬╡_1 := | x_1 - sqrt(a) | тЙд 2**(e-4.5)  -- special case, see above
            xn = (xn + a / xn) >> 1; // ╬╡_2 := | x_2 - sqrt(a) | тЙд 2**(e-9)    -- general case with k = 4.5
            xn = (xn + a / xn) >> 1; // ╬╡_3 := | x_3 - sqrt(a) | тЙд 2**(e-18)   -- general case with k = 9
            xn = (xn + a / xn) >> 1; // ╬╡_4 := | x_4 - sqrt(a) | тЙд 2**(e-36)   -- general case with k = 18
            xn = (xn + a / xn) >> 1; // ╬╡_5 := | x_5 - sqrt(a) | тЙд 2**(e-72)   -- general case with k = 36
            xn = (xn + a / xn) >> 1; // ╬╡_6 := | x_6 - sqrt(a) | тЙд 2**(e-144)  -- general case with k = 72

            // Because e тЙд 128 (as discussed during the first estimation phase), we know have reached a precision
            // ╬╡_6 тЙд 2**(e-144) < 1. Given we're operating on integers, then we can ensure that xn is now either
            // sqrt(a) or sqrt(a) + 1.
...
library Bytes {
     ...
     *
     * NOTE: replicates the behavior of https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/indexOf[Javascript's 
`Array.indexOf`]
     */
...
`Array.indexOf`]
     ...
     * * If `s` is not present in the buffer, returns type(uint256).max
     *
     * NOTE: replicates the behavior of https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/lastIndexOf[Javascript's 
`Array.lastIndexOf`]
     */
...
`Array.lastIndexOf`]
     ...
     * memory.
     *
     * NOTE: replicates the behavior of https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/slice[Javascript's 
`Array.slice`]
     */
...
`Array.slice`]
     ...
     *
     * Example: hex"01234567" тЖТ hex"0001020304050607"
     */
...
library Strings {
    ...
    function _tryParseIntUncheckedBounds(
        ...
        bytes1 sign = begin == end ? bytes1(0) : bytes1(_unsafeReadBytesOffset(buffer, begin)); // don't do out-of-bound (possibly unsafe) read if sub-string 
is empty
    ...
    function _tryParseHexUintUncheckedBounds(
        ...
        bool hasPrefix = (end > begin + 1) && bytes2(_unsafeReadBytesOffset(buffer, begin)) == bytes2("0x"); // don't do out-of-bound (possibly unsafe) read 
if sub-string is empty
    ...
    function tryParseAddress(
        ...
        bool hasPrefix = (end > begin + 1) && bytes2(_unsafeReadBytesOffset(bytes(input), begin)) == bytes2("0x"); // don't do out-of-bound (possibly unsafe) 
read if sub-string is empty
